...
import { Mic, MicOff, Video, VideoOff, PhoneOff, User, Volume2, Wifi, ScreenShare, ScreenShareOff, Circle, Square } from "lucide-react";
...
interface CallScreenProps {
  ...
  /** PHASE 10: Screen sharing — video calls only, feature-detected. */
  screenSharing?: boolean;
  peerScreenSharing?: boolean;
  onToggleScreenShare?: () => void;
  screenShareSupported?: boolean;
  /** PHASE 10: Call recording — audio-only, works for voice + video calls. */
  recording?: boolean;
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 relative flex items-center justify-center overflow-hidden">
        ...
        {isVideo && peerScreenSharing && (
          <div className="absolute top-14 left-4 bg-brand/80 rounded-full px-3 py-1.5 text-xs font-medium text-white flex items-center gap-1.5">
            <ScreenShare size={12} />
            {peerLabel} screen share kar rahe hain
          </div>
        )}

        {isVideo && screenSharing && (
          <div className="absolute top-14 right-4 bg-brand/80 rounded-full px-3 py-1.5 text-xs font-medium text-white flex items-center gap-1.5">
            <ScreenShare size={12} />
            Aap screen share kar rahe hain
          </div>
        )}

        {(recording || peerRecording) && (
          <div
            className={`absolute ${isVideo ? "top-24 right-4" : "top-4 right-4"} bg-red-600/90 rounded-full px-3 py-1.5 text-xs font-medium text-white flex items-center gap-1.5`}
          >
            <span className="w-2 h-2 rounded-full bg-white animate-pulse" />
            {recording ? "Recording" : `${peerLabel} record kar rahe hain`}
          </div>
        )}
      ...
      <div className="p-6 flex items-center justify-center gap-5 shrink-0">
        ...
        {isVideo && screenShareSupported && (
          <button
            ...
            disabled={phase !== "active"}
            className={`w-14 h-14 rounded-full flex items-center justify-center disabled:opacity-40 transition ${
              screenSharing ? "bg-brand text-white" : "bg-slate-800 text-white hover:bg-slate-700"
            }`}
        ...
        <button
          ...
          disabled={phase !== "active"}
          className={`w-14 h-14 rounded-full flex items-center justify-center disabled:opacity-40 transition ${
            recording ? "bg-red-600 text-white" : "bg-slate-800 text-white hover:bg-slate-700"
          }`}
          aria-label={recording ? "Stop recording" : "Start recording"}
        >
          {recording ? <Square size={18} /> : <Circle size={20} className="fill-red-500 text-red-500" />}
        </button>
