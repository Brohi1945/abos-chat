...
function useRingtone() {
  useEffect(() => {
    ...
    // Gentle two-tone ring (like a classic phone ringback) instead of a
    // single sharp, loud tone — much easier on the ears, especially
    // through a phone speaker.
    const RING_FREQUENCIES = [480, 620];
    const PEAK_GAIN = 0.12;

    const playTone = (startOffset: number, duration: number) => {
      if (!ctx) return;
      const start = ctx.currentTime + startOffset;
      for (const freq of RING_FREQUENCIES) {
        const osc = ctx.createOscillator();
        const gain = ctx.createGain();
        osc.type = "sine";
        osc.frequency.value = freq;
        gain.gain.setValueAtTime(0.0001, start);
        gain.gain.exponentialRampToValueAtTime(PEAK_GAIN, start + 0.08);
        gain.gain.setValueAtTime(PEAK_GAIN, start + duration - 0.08);
