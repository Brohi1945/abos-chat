...
// ============================================================
//  PHASE 10: Live Adaptive Quality
// ============================================================
// Builds directly on the quality numbers getCallQualityReport() was
// already computing every 3s (previously only used to show a "weak
// connection" toast). Now the SAME tier also drives the video sender's
// actual encoding — lower bitrate + a smaller encoded resolution — so a
// bad network genuinely gets a smoother, less choppy call instead of
// just a warning. Audio is left alone: Opus at ~32kbps (tuneOpusAudio)
// is already small enough that congestion is almost always a video cost.
...
const QUALITY_TIERS: Record<CallQualityReport["quality"], QualityTierSettings> = {
  excellent: { maxBitrate: 2_500_000, scaleResolutionDownBy: 1 },
  good: { maxBitrate: 1_500_000, scaleResolutionDownBy: 1 },
  poor: { maxBitrate: 700_000, scaleResolutionDownBy: 2 },
  "very-poor": { maxBitrate: 250_000, scaleResolutionDownBy: 4 },
};

// Module-level like qualityMonitorInterval above — this app only ever
// runs one active call at a time, so a single "what tier is currently
// applied" slot is enough and avoids threading extra state through
// every caller of startQualityMonitoring().
let lastAppliedTier: CallQualityReport["quality"] | null = null;

function applyAdaptiveVideoQuality(pc: RTCPeerConnection, quality: CallQualityReport["quality"]): void {
  if (quality === lastAppliedTier) return; // no change — skip a pointless setParameters() round trip

  const sender = pc.getSenders().find((s) => s.track && s.track.kind === "video");
  if (!sender) {
    // Voice call — nothing to adapt, but still remember the tier so we
    // don't re-check on every single poll.
    lastAppliedTier = quality;
    return;
  }
  ...
  try {
    ...
    sender
      ...
      .then(() => {
        lastAppliedTier = quality;
        console.log(`✅ Adaptive quality: "${quality}" tier applied`, tier);
      })
      .catch((err) => {
        console.warn("Adaptive quality: setParameters failed:", err);
      });
  } catch (err) {
    console.warn("Adaptive quality error:", err);
  }
}
  ...
  setTimeout(() => {
    qualityMonitorInterval = window.setInterval(async () => {
      try {
        ...
        applyAdaptiveVideoQuality(pc, report.quality);
        onReport(report);
      } catch (err) {
...
// ============================================================
// getDisplayMedia() is desktop-browser territory (Chrome/Firefox/Edge on
// Windows/Mac/Linux) — most mobile browsers (iOS Safari, and Android
// Chrome in normal tabs) don't expose it at all. isScreenShareSupported()
// is the single source of truth for whether to even show the UI button,
// so unsupported devices never see a control that would just throw.
...
export async function getScreenStream(): Promise<MediaStream> {
  ...
  // Tells the browser's encoder to favor sharpness over smooth motion —
  // right call for sharing product photos/text/UI rather than a camera
  // feed. Screen share track only; camera keeps browser defaults.
  const track = stream.getVideoTracks()[0];
  if (track && "contentHint" in track) {
    try {
      track.contentHint = "detail";
    } catch {
      // not fatal — just a quality hint
    }
  }
  return stream;
}
...
// ============================================================
//  PHASE 10: Shared audio-recording mime type picker
// ============================================================
// Same probing list ChatWindow.tsx's voice-note recorder already uses —
// centralized here so voicemail and call-recording (both new, both in
// CallManager.tsx) share one implementation instead of two more copies.
