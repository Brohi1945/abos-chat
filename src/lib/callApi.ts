...
export type SignalMessage =
  ...
  // PHASE 10: Screen sharing — pure UI status ping (who's sharing right
  // now), no SDP/renegotiation involved. The video track swap itself
  // happens via RTCRtpSender.replaceTrack() on the existing video m-line,
  // so it needs no offer/answer at all — this message only exists so the
  // OTHER side can show a "peer is sharing their screen" badge.
  | { type: "screen-share"; active: boolean; from: string }
  // PHASE 10: Call recording — same idea as screen-share above: a pure
  // status ping so the OTHER party sees a "this call is being recorded"
  // notice. The actual recording is local-only (Web Audio mix of both
  // parties' audio tracks via a MediaRecorder), no SDP involved.
  | { type: "recording-status"; active: boolean; from: string };
