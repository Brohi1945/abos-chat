...
export function staffIdentity(me: Profile): {
  senderName?: string;
...
interface SendMessageInput {
  ...
  replyToId?: string;
  // PHASE 10: links a voicemail/recording message back to the call it
  // came from (abos_chat_messages.call_id already existed — endCall()'s
  // own call-log insert has always set it directly; this just lets the
  // general-purpose sendMessage() set it too, for voicemail/recording).
  callId?: string;
}
...
//  Purely informational — doesn't block anything. The "update own
//  profile" RLS policy already restricts rows to auth.uid() = id, so
//  calling this with anyone else's id is a silent no-op (0 rows
//  affected), never an error and never someone else's status.
// ============================================================

export async function setAwayStatus(userId: string, isAway: boolean): Promise<boolean> {
  const { error } = await supabase
    .from("abos_chat_profiles")
...
//  Purely a filter/organization tool — does not restrict who can
//  reply. Pass staffId = null to unassign.
// ============================================================
...
//  Lives in its own table (abos_chat_conversation_notes) with its own
//  RLS that never references customer_id — a customer session can
//  never read these no matter what the frontend does. These
//  functions must never be called from CustomerChatScreen.tsx or any
//  code path a customer session renders.
// ============================================================

export async function listNotes(conversationId: string): Promise<ConversationNote[]> {
  const { data, error } = await supabase
    .from("abos_chat_conversation_notes")
    .select("*")
    .eq("conversation_id", conversationId)
    .order("created_at", { ascending: false });
  if (error) {
  ...
  // Two-step fetch + client-side join (same pattern already used by
  // getReactionsForMessages' .in() lookup below) instead of a
  // PostgREST FK-embed — this codebase doesn't use embeds anywhere
  // else, so this keeps the query style consistent and avoids any
  // relationship-detection surprises for a table that was just created.
  ...
  const nameById = new Map((authors || []).map((a: any) => [a.id, a.name as string | null]));

  return rows.map((row: any) => ({
    id: row.id,
    conversation_id: row.conversation_id,
    author_id: row.author_id,
    author_name: nameById.get(row.author_id) ?? null,
    body: row.body,
...
export async function addNote(
  ...
  const { error } = await supabase
    .from("abos_chat_conversation_notes")
    .insert({ conversation_id: conversationId, author_id: authorId, body: trimmed });
  if (error) {
    console.error(error);
    return false;
  }
...
export async function deleteNote(noteId: string): Promise<boolean> {
  const { error } = await supabase.from("abos_chat_conversation_notes").delete().eq("id", noteId);
  if (error) {
    console.error(error);
    return false;
  }
